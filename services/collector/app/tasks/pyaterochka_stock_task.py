"""Celery task: collect stock availability for a Pyaterochka SKU."""
...
@celery_app.task(bind=True, max_retries=3, name="pyaterochka.collect_stock")
def collect_pyaterochka_stock(self, sku_platform_id: str) -> None:
    """Collect stock availability for one Pyaterochka SKUPlatform."""
    ...
    if row is None:
        logger.warning("collect_pyaterochka_stock: sku_platform %s not found", sku_platform_id)
        return
    ...
    with get_db_session() as db:
        ...
        try:
            ...
            if exc.code == "NOT_FOUND":
                return
            logger.warning(
                "collect_pyaterochka_stock: ScraperError code=%s sku_platform=%s",
    ...
    logger.info(
        "collect_pyaterochka_stock: done sku_platform=%s scraper_level=%s",
