"""Celery task: collect current price for a Magnit SKU."""
...
@celery_app.task(bind=True, max_retries=3, name="magnit.collect_price")
def collect_magnit_price(self, sku_platform_id: str) -> None:
    """Collect current price snapshot for one Magnit SKUPlatform."""
    ...
    if row is None:
        logger.warning("collect_magnit_price: sku_platform %s not found", sku_platform_id)
        return
    ...
    with get_db_session() as db:
        ...
        try:
            ...
            if exc.code == "NOT_FOUND":
                return
            logger.warning(
                "collect_magnit_price: ScraperError code=%s sku_platform=%s",
    ...
    logger.info(
        "collect_magnit_price: done sku_platform=%s scraper_level=%s",
