...
Downloads the main product image asynchronously and stores it in MinIO (S3).
...
Key design decisions:
  ...
  - Image URL validated against _OZ_IMAGE_CDN_RE SSRF allowlist before download.
...
def collect_ozon_content(self, sku_platform_id: str) -> None:
    ...
    sp_id, sku_id, raw_item_id, platform_id, org_id = row
    ...
    with get_db_session() as db:
        ...
        try:
            ...
            if exc.code == "NOT_FOUND":
                logger.info(
                    "collect_ozon_content: product sku_platform=%s not found — skipping",
                    sku_platform_id,
                )
                return
            logger.warning(
    ...
    logger.info(
        "collect_ozon_content: done item_id=%s scraper_level=%s", item_id, scraper_level
    )
