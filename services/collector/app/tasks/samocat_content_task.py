...
Key design decisions:
  ...
  - Image URL validated against _SK_IMAGE_CDN_RE SSRF allowlist before download.
...
from app.scrapers.samocat import _download_image_async, _SK_IMAGE_CDN_RE
...
def collect_samocat_content(self, sku_platform_id: str) -> None:
    ...
    logger.info(
        "collect_samocat_content: product_id=%r (numeric=%s) sku_platform=%s",
        product_id,
        product_id.isdigit(),
        sku_platform_id,
    )
    ...
    with get_db_session() as db:
        ...
        try:
            ...
            if exc.code == "NOT_FOUND":
                logger.info(
                    "collect_samocat_content: product sku_platform=%s not found — skipping",
                    sku_platform_id,
                )
                return
            logger.warning(
    ...
    logger.info(
        "collect_samocat_content: done sku_platform=%s scraper_level=%s",
        sku_platform_id,
        scraper_level,
    )
