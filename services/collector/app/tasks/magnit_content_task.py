"""Celery task: collect content for a Magnit SKU."""
...
from app.scrapers.magnit import _download_image_async
...
@celery_app.task(bind=True, max_retries=3, name="magnit.collect_content")
def collect_magnit_content(self, sku_platform_id: str) -> None:
    """Collect content (title, description, composition, image) for one Magnit SKUPlatform."""
    ...
    if row is None:
        logger.warning("collect_magnit_content: sku_platform %s not found", sku_platform_id)
        return
    ...
    with get_db_session() as db:
        ...
        try:
            ...
            if exc.code == "NOT_FOUND":
                return
            logger.warning(
                "collect_magnit_content: ScraperError code=%s sku_platform=%s",
    ...
    logger.info(
        "collect_magnit_content: scraped sku_platform=%s title_len=%d desc_len=%d image_url=%r",
        sku_platform_id,
    ...
    async def _fetch_image():
        ...
        if content.image_url:
            try:
                ...
                logger.warning("collect_magnit_content: image SSRF reject sku_platform=%s: %s", sku_platform_id, exc)
            except Exception as exc:  # noqa: BLE001
                logger.warning("collect_magnit_content: image download failed sku_platform=%s: %s", sku_platform_id, exc)
        return img
    ...
    if image_bytes is not None:
        try:
            s3_key = f"org/{org_id}/sku/{sku_id}/magnit/main.jpg"
            minio = _get_minio()
            minio.upload(s3_key, image_bytes, "image/jpeg")
        except Exception:  # noqa: BLE001
            logger.warning(
                "collect_magnit_content: MinIO upload failed for sku_platform=%s",
    ...
    logger.info(
        "collect_magnit_content: done sku_platform=%s scraper_level=%s",
