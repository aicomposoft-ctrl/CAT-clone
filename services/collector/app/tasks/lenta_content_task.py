...
Fetches product title, description, composition, and image from the Lenta
API (`GET /api/v1/products/{product_id}`).  Downloads the main product image
...
Key design decisions:
  ...
  - Image URL validated against _LT_IMAGE_CDN_RE SSRF allowlist before
...
def collect_lenta_content(self, sku_platform_id: str) -> None:
    ...
    if image_bytes is not None:
        try:
            ...
            logger.warning(
                "collect_lenta_content: MinIO upload failed for sku_platform=%s",
                sku_platform_id,
                exc_info=True,
            )
            s3_key = None
        ...
        logger.warning(
            "collect_lenta_content: image download failed for sku_platform=%s",
            sku_platform_id,
        )
